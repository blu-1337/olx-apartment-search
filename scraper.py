...
Fetches apartment listings from OLX.ro, filters by price and description,
...
if __name__ == "__main__":
    ...
    # Process listings
    current_listings = scraper.process_listings()
    print(f"\nTotal valid listings found: {len(current_listings)}")
    
    # Find new listings
    new_listings = scraper.find_new_listings(current_listings)
    print(f"New listings: {len(new_listings)}")
    
    # Update database
    if new_listings:
        scraper.update_database(new_listings)
        print(f"Database updated with {len(new_listings)} new listings")
        
        # Sort listings by price (ascending) before sending
        new_listings_sorted = sorted(new_listings, key=lambda x: x.get('price', 0))
